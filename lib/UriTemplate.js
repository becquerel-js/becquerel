...
module.exports = class UriTemplate {
    ...
    _getVariableParsers() {
        ...
        return this.toArray().reduce((accumulator, component, position) => {
            ...
            const pattern = component.replace(varRegEx, '(.+)');
            const varName = varRegEx.exec(component)[1];

            return [...accumulator, {name: varName, position, pattern}];
        }, []);
    }
    ...
    getVariables(path = '') {
        ...
        const varParsers = this._getVariableParsers();
        ...
        return components.reduce((accumulator, component, position) => {
            ...
            const value = new RegExp(parser.pattern).exec(component)[1];
